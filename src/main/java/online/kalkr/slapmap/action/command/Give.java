...
public class Give {
    public static void giveStick(PlayerEntity player, String name) {
        ...
        slapstick.setCustomName(Text.literal("Slapstick - "+ name));
    ...
    public static int give(CommandContext<ServerCommandSource> c) throws CommandSyntaxException {
        ...
        if (!Slapmap.mapManager.has(name)) {
            src.sendFeedback(Text.literal("There isn't an image named "+ name +"!").formatted(Formatting.RED), false);
            src.sendFeedback(Text.literal("Try importing one using: /slap "+ name +" url=...").formatted(Formatting.GRAY, Formatting.ITALIC), false);
            return -1;
        }
